...
import { Book, Customer, CartItem } from "@/types";
import { SearchBar } from "@/components/SearchBar";
import { formatRupiah } from "@/lib/utils";
import { ArrowLeft, ShoppingCart, Plus, Minus, Trash2, Save, User, Search } from "lucide-react";
                ...
                {cart.map((c) => (
                  <div key={c.book.id} className="flex items-center justify-between py-2 px-3 rounded-xl bg-brand-50/60">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-brand-950 w-8 text-center">{c.quantity}</span>
                      <button
                        onClick={() => updateQty(c.book.id, 1)}
                        className="p-1 rounded-lg hover:bg-brand-100 text-brand-600"
                      >
